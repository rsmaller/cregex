...
testCases = [
	...
	r"(?=abc)a",
	r"a(?<=abc)",
	r"\b\w{3}", r"\w{3}\b",	r"\b\w{3}\b", r"(\w{3}$)", r"(\w{3})$",
	r"(^\w{3})", r"^(\w{3})",
	r"\w(?=$)",	r"(?<=^)\w",
	r"r? ?",
	r"(?:ular)? ?",
	...
	r"[Rr]eg(?:ular)? ?[Ee]x(?:p|pr|pression)?",
	r"[Rr]eg(?:ular)? ?[Ee]x(?:p|pr|pression)?\b",
	r"[Rr]eg(?:ular)?[Ee]x(?:p|pr|pression)?\b",
	r"\b[Rr]eg(?:ular)? ?[Ee]x(?:p|pr|pression)?",
	...
	r"(?=a{3})b", r"b(?<=a{3})", r"(?<=\w{3})b", r"(\w{3})( \d{3})",
	r"(\w{3})(?= )",
	r"\w{3}(?= )",
	r"(\w{3}) ",
